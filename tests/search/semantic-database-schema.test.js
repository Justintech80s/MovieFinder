...
test('Phase 5 migration creates provenance-aware semantic documents with English full-text search and 1536-d vectors', async () => {
  ...
  assert.match(sql, /embedding\s+extensions\.vector\(1536\)/);
  assert.match(sql, /to_tsvector\('english'/);
  assert.match(sql, /using gin\s*\(fts\)/);
  assert.match(sql, /using hnsw\s*\(embedding extensions\.vector_cosine_ops\)/);
  assert.match(sql, /enable row level security/);
  assert.match(sql, /revoke all on table public\.cinema_documents from public, anon, authenticated/);
});

test('Phase 5 hybrid search RPC is bounded and combines lexical and semantic ranks with RRF k=60', async () => {
  const sql = await migrationSql();

  assert.match(sql, /search_cinema_documents\(\s*query_text text,\s*query_embedding extensions\.vector\(1536\)/);
