...
test('configured Python service receives person-search intent and returns its response', async () => {
  ...
  const fetchImpl=async (url,options)=>{
    request={url,options};
    return {ok:true,status:200,json:async()=>({person:{name:'Quentin Tarantino'},filmography:[{title:'Pulp Fiction',roles:['director','writer','cast']}],results:[],availabilitySummary:{available:0,unavailable:0,unknown:1},verified:true})};
  };
