...
create index if not exists idx_cinema_documents_embedding_hnsw
  on public.cinema_documents using hnsw (embedding extensions.vector_cosine_ops)
  where embedding is not null;

alter table public.cinema_documents enable row level security;

revoke all on table public.cinema_documents from public, anon, authenticated;
...
revoke all on function public.search_cinema_documents(text, extensions.vector, integer) from public, anon, authenticated;
grant execute on function public.search_cinema_documents(text, extensions.vector, integer) to service_role;
