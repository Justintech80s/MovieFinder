...
const label=v=>typeof v==='object'?(v.name||v.label||v.title||''):v;
const values=v=>v==null?[]:Array.isArray(v)?v:[v];
const fields=movie=>[movie.title,movie.description,...values(movie.genres),...values(movie.tags),...values(movie.actors??movie.cast),...values(movie.directors??movie.director),...values(movie.writers??movie.writer),...values(movie.themes),...values(movie.styles),...values(movie.movements),movie.year].map(label).join(' ');
...
const phraseInQuery=(query,value)=>{const q=normalizeSearchText(query),l=normalizeSearchText(value);return Boolean(l&&q.includes(l));};
function typedGraphSignal(movie,intent={}){
 ...
 const outgoing=traverseCinemaGraph(graph,movieId,{maxDepth:1});
 const actorPaths=graph.edges().filter(e=>e.to===movieId&&e.type==='STARS').map(edge=>({from:graph.getNode(edge.from),edge,node:graph.getNode(edge.from),depth:1}));
 const paths=[...outgoing,...actorPaths];
