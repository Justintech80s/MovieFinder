...
function buildSearchExplanation(movie,intent,why){
  const title=String(movie?.title||'This title');
  const details=[];
  const concepts=Array.isArray(intent?.concepts)?intent.concepts:[];
  const discoveryTerms=Array.isArray(intent?.discoveryTerms)?intent.discoveryTerms:[];
  const conceptText=[...new Set([...concepts,...discoveryTerms])].slice(0,4).join(', ');
  if(conceptText) details.push(`It connects to your search through ${conceptText}.`);
  const description=String(movie?.description||'').trim();
  if(description) details.push(description.endsWith('.')?description:`${description}.`);
  const ratings=[];
  const imdb=Number(movie?.ratings?.imdb);
  const rt=Number(movie?.ratings?.rottenTomatoes);
  if(Number.isFinite(imdb)) ratings.push(`IMDb ${imdb.toFixed(1)}`);
  if(Number.isFinite(rt)) ratings.push(`Rotten Tomatoes ${Math.round(rt)}%`);
  if(ratings.length) details.push(`Ratings: ${ratings.join(' and ')}.`);
  const providers=[...new Set((movie?.offers||[]).map(o=>o?.provider).filter(Boolean))].slice(0,3);
  if(providers.length) details.push(`Current availability includes ${providers.join(', ')}.`);
  if(why?.length) details.push(`MovieFinder match signals: ${why.slice(0,4).join(', ')}.`);
  return `${title}: ${details.join(' ')}`.trim();
}
...
function rankingBonus(movie,intent={}){
  ...
  if(mode==='recent'){
    ...
    const recency=Number.isFinite(year)?clamp01((year-2000)/30):0;
    return {score:recency*.34,reason:Number.isFinite(year)?'recent release signal':null};
  }
  if(mode==='popular'){
    return {score:(voteNorm*.30)+(quality*.06),reason:voteNorm>0?'popularity signal':null};
  }
...
export function rankResults(results=[],intent={}){
  return results.map(m=>{
    ...
    if(m.offers?.length){score+=(intent.rankingIntent==='highest-rated'?.04:.14);why.push('current availability');}
    ...
    return {
      ...
      searchExplanation:buildSearchExplanation(m,intent,why)
    };
