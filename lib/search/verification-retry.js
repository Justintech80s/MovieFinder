...
export async function runBoundedVerificationRetry({
  firstResponse={},
  parsedIntent={},
  retrySearch=null
}={}){
  const firstQuality=assessSearchQuality({parsedIntent,results:firstResponse?.results});
  if(!firstQuality.retry || typeof retrySearch!=='function'){
    return {
      ...firstResponse,
      verificationRetry:{
        attempted:false,
  ...
  const retryResponse=await retrySearch();
  return chooseVerifiedResponse(firstResponse,retryResponse,{parsedIntent});
}
