...
export function ingestMovieMetadata(graph,movie,defaults={}){
 ...
 for(const person of normalized.people){
  ...
  if(role==='actor')graph.addEdge(node.id,movieNode.id,{type:'STARS',...options});
  if(role==='director')graph.addEdge(movieNode.id,node.id,{type:'DIRECTED_BY',...options});
  if(role==='writer')graph.addEdge(movieNode.id,node.id,{type:'WRITTEN_BY',...options});
 }
