...
const providerLogo=o=>{
  ...
  if(!raw) return null;
  const value=String(raw).replace('{profile}','s100').replace('{format}','png');
...
const safeUrl=value=>{
  ...
  try{
    ...
    if(host==='localhost'||host==='127.0.0.1'||host==='0.0.0.0'||host==='::1'||host.endsWith('.local')) return null;
    return url.toString();
  }catch{return null;}
};
const linkSpecificity=value=>{
  if(!value) return 'NONE';
  try{
    const url=new URL(value);
    const path=url.pathname.replace(/\/+$/,'');
    return path&&path!=='/'?'TITLE':'GENERIC';
  }catch{return 'NONE';}
};
export function isAvailabilityFresh(checkedAt,now=new Date().toISOString(),ttlMs=5*60*1000){
  const checked=Date.parse(checkedAt);
  const current=Date.parse(now);
  if(!Number.isFinite(checked)||!Number.isFinite(current)) return false;
  const age=current-checked;
  return age>=0&&age<=ttlMs;
}
const QUALITY_ALIASES={_4K:'4K',UHD:'4K',BLURAY:'Blu-ray',BLURAY_4K:'4K Blu-ray',DVD:'DVD',SD:'SD',HD:'HD'};
const normalizeQuality=value=>{
  const raw=String(value||'').trim().toUpperCase();
  return QUALITY_ALIASES[raw]||String(value||'').trim()||null;
};
const qualityRank={SD:0,HD:1,'4K':2,'Blu-ray':3,'4K Blu-ray':4,DVD:5};
...
export function normalizeOffers(raw=[], options={}){
  return (raw||[]).map(o=>{
    ...
    const offer={
      ...
      quality:normalizeQuality(o.quality||o.presentationType),
      url:safeUrl(o.url||o.standardWebURL||o.webUrl||null)
    };
    const specificity=linkSpecificity(offer.url);
    return {
      ...offer,
      linkSpecificity:specificity,
      titleSpecificUrl:specificity==='TITLE'?offer.url:null,
      timeline:toTimelineEntry(offer,{checkedAt:options.checkedAt,current:true,source:options.source,region:options.region})
    };
  });
}
...
export function filterOffers(offers,intent={}){
  return (offers||[]).filter(o=>{
    if(intent.provider&&!norm(o.provider).includes(norm(intent.provider))) return false;
    if(intent.freeOnly&&!['FREE','ADS'].includes(o.type)) return false;
    if(intent.rentOnly&&o.type!=='RENT') return false;
    if(intent.buyOnly&&o.type!=='BUY') return false;
    if(intent.maxPrice!=null){
      const price=Number(o.price);
      if(!Number.isFinite(price)||price>Number(intent.maxPrice)) return false;
      if(o.currency&&String(o.currency).toUpperCase()!=='USD') return false;
    }
    return true;
  });
}
...
export function verifyAvailabilityOffers(offers=[],{
  checkedAt=null,
  now=new Date().toISOString(),
  ttlMs=5*60*1000
}={}){
  const fresh=isAvailabilityFresh(checkedAt,now,ttlMs);
  const confirmedOffers=[];
  const staleOffers=[];
  const uncertainOffers=[];

  for(const offer of offers||[]){
    if(!fresh){
      staleOffers.push(offer);
      continue;
    }
    if(offer?.linkSpecificity==='TITLE'&&offer?.url){
      confirmedOffers.push(offer);
      continue;
    }
    uncertainOffers.push(offer);
  }

  let status='UNKNOWN';
  if(confirmedOffers.length) status='CONFIRMED';
  else if(staleOffers.length) status='STALE';
  else if(uncertainOffers.length) status='UNCERTAIN';

  const confidence=
    status==='CONFIRMED'?0.95:
    status==='UNCERTAIN'?0.65:
    status==='STALE'?0.35:
    0.2;
