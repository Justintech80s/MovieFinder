...
export function parseIntent(query='') {
  ...
  if (person) return { raw, ...person, titleQuery:null, similarityTitle:null, provider, freeOnly, rentOnly, buyOnly, filmographyView, genreWords, rtMin, yearMin, yearMax, mediaType, requestedSeason, requestedEpisode };
  return { raw, kind:'catalog', personName:null, role:null, titleQuery:null, similarityTitle:relatedTitle, provider, freeOnly, rentOnly, buyOnly, filmographyView:null, genreWords, rtMin, yearMin, yearMax, mediaType, requestedSeason, requestedEpisode, confidence:0.7 };
}
