...
import { filterOffers } from './availability.js';
...
function constrainAvailabilityResult(result,intent={}){
  if(!result || result.error) return result;
  const movie=result.movie||result;
  const offers=Array.isArray(movie.offers)?filterOffers(movie.offers,intent):[];
  const constrained={
    ...movie,
    offers,
    streamingTimeline:offers.map(offer=>offer?.timeline).filter(Boolean),
    freeAvailable:offers.some(offer=>['FREE','ADS'].includes(offer?.type)),
    best:offers[0]||null
  };
  return result.movie?{...result,movie:constrained}:constrained;
}
...
export async function runPersonFilmographySearch(intent, options={}) {
  ...
  if(intent.filmographyView==='complete'){
    ...
    return {
      ...
      failedRoles:Array.isArray(resolved.failedRoles)?resolved.failedRoles:[]
    };
  }

  const requestedLimit=options.availabilityLimit==null?credits.length:Number(options.availabilityLimit);
  const availabilityLimit=Number.isFinite(requestedLimit)?Math.max(0,requestedLimit):credits.length;
  ...
  const records=credits.map((credit,i)=>{
    if(i<checked.length) return buildFilmographyRecord(credit,constrainAvailabilityResult(checked[i],intent));
