...
import { runBoundedVerificationRetry } from './verification-retry.js';
...
function explanationIsSubstantive(explanation, result, parsedIntent={}){
  const text=String(explanation||'').trim();
  if(text.length<80) return false;
  const words=text.toLowerCase().match(/[a-z0-9]+/g)||[];
  if(new Set(words).size<12) return false;
  const concepts=[...(parsedIntent.concepts||[]),...(parsedIntent.discoveryTerms||[])].map(v=>String(v).toLowerCase());
  const title=String(result?.title||'').toLowerCase();
  return (title&&text.toLowerCase().includes(title)) || concepts.some(term=>term&&text.toLowerCase().includes(term));
}

async function enrichVerifiedResults({ modelRouter, results, parsedIntent, evidence, structuredData = null, allowSynthesis = true }) {
  ...
  if(structuredData){
    ...
    synthesized={results:results.map((result,index)=>{
      const suggestion=result?.id!=null&&byId.has(String(result.id))?byId.get(String(result.id)):suggestions[index];
      if(!suggestion||typeof suggestion!=='object') return result;
      const ai={};
      for(const key of ['explanation','semanticTags','themes','connections','summary']) if(suggestion[key]!==undefined) ai[key]=suggestion[key];
      return Object.keys(ai).length?{...result,ai}:result;
    })};
  }else{
    if(!allowSynthesis || typeof modelRouter?.run !== 'function') return results;
    const enrichment=createAIEnrichment({modelRouter});
    synthesized=await enrichment.synthesize(results,{intent:parsedIntent},{evidence});
  }
  return synthesized.results.map(result=>{
    const explanation=typeof result?.ai?.explanation==='string'?result.ai.explanation.trim():'';
    return explanation&&explanationIsSubstantive(explanation,result,parsedIntent)
      ? {...result,searchExplanation:explanation}
      : result;
  });
}
...
async function addVerifiedAiReasoning({ modelRouter, query, parsedIntent, evidence, baseMode }) {
  ...
  try {
    ...
    return {
      ...
      structuredData: response?.output?.structuredData ?? null,
        ...
        try {
          ...
          if (Array.isArray(candidates) && candidates.length) {
            ...
            if (verified.movies.length) {
              ...
              return runBoundedVerificationRetry({
                firstResponse,
                parsedIntent,
                retrySearch:async()=>{
