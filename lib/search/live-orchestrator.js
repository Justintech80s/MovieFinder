...
export function createLiveOrchestrator({
  ...
  return {
    async search({ query = '', parsedIntent = {} } = {}) {
      ...
      if (parsedIntent.kind !== 'person-filmography' && typeof hybridRetriever?.search === 'function') {
        try {
          const candidates = await hybridRetriever.search({ query, parsedIntent });
          if (Array.isArray(candidates) && candidates.length) {
            const verified = await verifyGraphAvailability(candidates, parsedIntent, lookupAvailability);
