...
export function createWikidataIngestionService({ client, normalizer, repository, semanticSync, now = () => new Date() } = {}) {
  ...
  async function persistBatch(rawEntities, retrievedAt, stats, storedEntityKeys) {
    ...
    if (typeof semanticSync === 'function') {
      try {
        await semanticSync(normalized);
      } catch {
        // Semantic indexing is best-effort; canonical graph persistence remains authoritative.
      }
    }
