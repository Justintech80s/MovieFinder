...
export function createCinemaGraphRepository({ db } = {}) {
  ...
  async function getEntity(idOrCanonicalKey) {
    const value = String(idOrCanonicalKey || '');
    if (!value) return null;
    const canonical = throwIfError(await client.from('cinema_entities').select('*').eq('canonical_key', value).maybeSingle(), 'get cinema entity by canonical key');
    if (canonical) return canonical;
    return throwIfError(await client.from('cinema_entities').select('*').eq('id', value).maybeSingle(), 'get cinema entity by id');
  }
