const CLAIM_RELATIONS = {
  P161: { type: 'ACTED_IN', reverse: true },
  P57: { type: 'DIRECTED', reverse: true },
  P58: { type: 'WROTE', reverse: true },
  P162: { type: 'PRODUCED', reverse: true },
  P344: { type: 'SHOT_BY', reverse: true },
  P136: { type: 'HAS_GENRE', reverse: false },
  P495: { type: 'FROM_COUNTRY', reverse: false },
  P921: { type: 'HAS_THEME', reverse: false },
  P737: { type: 'INFLUENCED_BY', reverse: false }
};

const MOVIE_INSTANCE_IDS = new Set(['Q11424', 'Q24869', 'Q506240']);
const PERSON_INSTANCE_IDS = new Set(['Q5']);
const GENRE_INSTANCE_IDS = new Set(['Q201658']);
const COUNTRY_INSTANCE_IDS = new Set(['Q6256']);
...
function entityType(entity) {
  ...
  if (instances.some(id => GENRE_INSTANCE_IDS.has(id))) return 'Genre';
  if (instances.some(id => COUNTRY_INSTANCE_IDS.has(id))) return 'Country';
  return 'Entity';
}
...
export function normalizeWikidataBatch({ entities = {}, retrievedAt } = {}) {
  ...
  for (const entity of Object.values(entities)) {
    ...
    for (const [property, config] of Object.entries(CLAIM_RELATIONS)) {
      for (const targetQid of itemIds(entity, property)) {
        const sourceKey = `wikidata:${entity.id}`;
        const targetKey = `wikidata:${targetQid}`;
        const relation = {
          fromCanonicalKey: config.reverse ? targetKey : sourceKey,
          toCanonicalKey: config.reverse ? sourceKey : targetKey,
