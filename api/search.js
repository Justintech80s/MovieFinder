...
async function jwSeasonOffers(id,seasonNumber){
  if(!id||!Number.isInteger(Number(seasonNumber))) return null;
  const controller=new AbortController();
  const timeout=setTimeout(()=>controller.abort(),JUSTWATCH_TIMEOUT_MS);
  try{
    const r=await fetch(JW,{method:'POST',headers:{'content-type':'application/json','accept':'application/json'},body:JSON.stringify({query:JUSTWATCH_SEASON_QUERY,variables:{id,country:'US',language:'en'}}),signal:controller.signal});
    if(!r.ok) return null;
    const d=await r.json();
    if(d.errors?.length) return null;
    const season=(d.data?.node?.seasons||[]).find(item=>Number(item?.seasonNumber)===Number(seasonNumber));
    ...
    const checkedAt=new Date().toISOString();
    return dedupeOffers(normalizeOffers((season.offers||[]).map(o=>({provider:o.package?.clearName||o.package?.shortName||o.package?.technicalName,providerLogo:o.package?.icon,type:o.monetizationType,price:o.retailPriceValue??o.retailPrice,currency:o.currency,quality:o.presentationType,url:o.standardWebURL})),{checkedAt,source:'JustWatch season availability'}));
  }catch{return null;}finally{clearTimeout(timeout);}
}
...
async function deterministicApplicationSearch({query='',parsedIntent={}}={}){
  ...
  if(parsed.requestedSeason!=null){
    results=await Promise.all(results.map(async movie=>{
