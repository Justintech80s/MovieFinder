...
async function availabilityForCredit(credit,intent){
  const nodes=await jwSearch(credit.title,12);const mapped=nodes.map(mapNode).filter(x=>x.mediaType==='MOVIE');const lowerTitle=String(credit.title||'').toLowerCase();const exact=mapped.filter(x=>x.title?.toLowerCase()===lowerTitle&&(!credit.year||!x.year||Math.abs(x.year-credit.year)<=1));const candidates=exact.length?exact:mapped.filter(x=>x.title?.toLowerCase()===lowerTitle);const movie=candidates[0];if(!movie)return null;const matching=filterOffers(movie.offers,intent);if(!matching.length)return null;return {...movie,offers:matching,streamingTimeline:matching.map(o=>o.timeline).filter(Boolean),best:bestOffer(matching),freeAvailable:matching.some(o=>['FREE','ADS'].includes(o.type)),personCredit:credit,dataConfidence:.91};
}
function availabilityFailure(error){return /^availability source\b/i.test(String(error?.message||error||''));}

async function javascriptPersonSearch(parsed){return runPersonFilmographySearch(parsed,{resolveCredits:resolvePersonCredits,lookupAvailability:credit=>availabilityForCredit(credit,parsed),rank:rankResults,availabilityLimit:60,concurrency:6});}

export default async function handler(req,res){
  try{
    const q=String(req.query?.q||'').trim();if(!q)return res.status(400).json({error:'Missing q'});const parsed=parseIntent(q);parsed.concepts=extractCinemaConcepts(q);
    if(parsed.kind==='person-filmography'){
      const pythonSearch=await runPythonPersonSearch(parsed);
      const personSearch=pythonSearch||await javascriptPersonSearch(parsed);
    ...
    results=results.filter(movie=>matchesHardConstraints(movie,parsed));results=rankResults(results,parsed).slice(0,40);return res.status(200).json({parsed,results,liveAt:new Date().toISOString()});
  }catch(error){if(availabilityFailure(error))return res.status(503).json({error:'Streaming availability temporarily unavailable',code:'AVAILABILITY_UNAVAILABLE',detail:String(error?.message||error)});return res.status(500).json({error:'MovieFinder search failed',detail:String(error?.message||error)});}
}
